...
AlbumManager::AlbumManager(QWidget* parent) : QDockWidget(parent)
      {
      // window
      ...
      for (int i = 0; i < int(m_items.size()); i++) {
            for (int j = 0; j < int(m_items.size()); j++) {
                  if (m_items.at(j)->albumItem.score()->title() != scoreList->item(j, 0)->text()) {
                        int h = scoreList->row(scoreList->findItems(m_items.at(j)->albumItem.score()->title(), Qt::MatchExactly).first());
