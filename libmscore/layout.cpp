...
void LayoutContext::collectPage()
      ...
      qreal headerFooterPadding = currentScore->styleP(Sid::staffHeaderFooterPadding);
      //qreal y = 0.0;
      // ---- before patch ---
      ...
      // collect and add systems to the page
      // (until the page is filled or a page break is encountered)
      for (;;) {
            ...
            if (!breakPage) {
                  ...
                  breakPage = (y + dist) >= endY && breakPages; // checks if the page is filled
                  if (!breakPage && breakPages && !titleAtTheBottom && curSystem->measure(0)->isVBox()) {
                        breakPage = (y + dist + currentScore->systems().value(systemIdx)->height()) >= endY;
                        }
                  }
            if (breakPage) {
                  ...
                  if (collected) { // FIXME: collected overused
                        pageOldMeasure = nullptr;
                        continuing = true;
                        }
                  break;
...
LayoutContext::LayoutContext(Score* s)
      ...
      {
      firstSystemIndent = currentScore && currentScore->styleB(Sid::enableIndentationOnFirstSystem);
      }
